...
function SetupScreen({onDone}){
  ...
  async function connect(){
    ...
    setBusy(true);setErr("");
    try{
      const r=await fetch(url.replace(/\/+$/,"")+"/rest/v1/profiles?limit=1",{headers:{"apikey":key,"Authorization":"Bearer "+key}});
      if(r.status===200||r.status===406||r.status===400){SB.save(url.replace(/\/+$/,""),key);onDone();}
    ...
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:16,background:"linear-gradient(135deg,#0F2744,#1E4D8C,#2E86DE)"}}>
      <div style={{width:"100%",maxWidth:440}}>
        ...
        <div style={{background:"#fff",borderRadius:20,padding:28,boxShadow:"0 24px 64px rgba(0,0,0,.25)"}}>
          ...
          <div style={{marginTop:14,fontSize:11,color:C.grey,textAlign:"center"}}>Get your credentials at supabase.com → Settings → API</div>
        </div>
      </div>
    </div>
  );
  ...
  return(
    <div style={{display:"flex",height:"100vh",fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"}}>
      ...
      <div style={{flex:1,overflowY:"auto",background:C.light}}>
        ...
        {page==="history"&&<PlaceholderPage icon="📝" title="History" desc="Your past reconciliation cycles will appear here once you connect Supabase."/>}
        {page==="clinical"&&<PlaceholderPage icon="🏥" title="Clinical Tools" desc="Calculators, minor ailments, lab values, global billing guide — coming in next update."/>}
        {page==="pricing"&&<PlaceholderPage icon="💳" title="Plans" desc="Basic $49 · Pro $99 · Enterprise $249 CAD/month. Contact support@narcosync.app"/>}
      </div>
    </div>
  );
}
  ...
  return(
    <div style={{padding:"28px 32px"}}>
      <div style={{marginBottom:24}}>
        ...
        <div style={{color:C.grey,fontSize:13,marginTop:4}}>{email} · Cycle V75</div>
      </div>
      <div style={{background:"linear-gradient(135deg,#FFFBEB,#FEF3C7)",border:"1.5px solid #FCD34D",borderRadius:14,padding:"16px 20px",marginBottom:24}}>
        ...
        <div style={{fontSize:12,color:C.grey,marginTop:2}}>Connect Supabase to save reconciliations · Add Stripe keys to enable billing</div>
      </div>
    ...
    <div style={{padding:"28px 32px"}}>
      ...
      {!done?(
        ...
        <div style={{background:"#fff",borderRadius:14,padding:24,boxShadow:"0 2px 10px rgba(0,0,0,.06)",textAlign:"center"}}>
          ...
          <div style={{fontSize:13,color:C.grey,marginBottom:20}}>NarcoSync is live on Vercel. Connect Supabase to save results.</div>
